...
# 🌟 3. Agregar el viaje completo al usuario y mostrar el usuario actualizado
agregar_viaje_a_usuario() {
  ...
  echo -e "${GREEN}📌 Respuesta de la API al agregar viaje:${NC}\n$(echo "$response" | jq .)\n"

  # 🌟 Mostrar el usuario actualizado con el viaje agregado
  echo -e "${BLUE}🔹 Obteniendo datos del usuario actualizado...${NC}"
  local usuario_actualizado=$(curl -s -X GET "$USUARIOS_ENDPOINT/$USER_ID" \
    -H "Authorization: Bearer $TOKEN")

  echo -e "${GREEN}📌 Usuario actualizado:${NC}\n$(echo "$usuario_actualizado" | jq .)\n"

  # 🌟 Mostrar el viaje actualizado después de ser agregado al usuario
  local viaje_id=$(echo "$VIAJE" | jq -r '.id')
  echo -e "${BLUE}🔹 Obteniendo datos del viaje actualizado...${NC}"
  local viaje_actualizado=$(curl -s -X GET "$VIAJES_ENDPOINT/$viaje_id" \
    -H "Authorization: Bearer $TOKEN")

  echo -e "${GREEN}📌 Viaje actualizado:${NC}\n$(echo "$viaje_actualizado" | jq .)\n"
}
...
quitar_viaje_de_usuario() {
  ...
  echo -e "${GREEN}📌 Respuesta de la API al quitar viaje:${NC}\n$(echo "$response" | jq .)\n"
}
